...
```bash
docker run --rm -p 127.0.0.1:3009:3009 ghcr.io/casys-ai/engineering-toolchain:0.3.1 \
  syson --port=3009 --hostname=0.0.0.0
docker run --rm -p 127.0.0.1:3014:3014 ghcr.io/casys-ai/engineering-toolchain:0.3.1 \
  build123d --port=3014 --hostname=0.0.0.0
docker run --rm -p 127.0.0.1:3015:3015 ghcr.io/casys-ai/engineering-toolchain:0.3.1 \
...
The image pins exact server versions — `mcp-syson@0.5.1`, `mcp-build123d@0.3.0`,
...
The `0.3.1` image is published for both `linux/amd64` and `linux/arm64`; Compose
...
```bash
docker build -t engineering-toolchain:local-0.3.1 .
docker run --rm engineering-toolchain:local-0.3.1 calculix --port=3015 --hostname=0.0.0.0
```

...
case "$SERVER" in
  syson)
    ...
    ;;
  build123d)
    exec deno run --config /opt/engineering-toolchain/deno.json --allow-all --cached-only --frozen jsr:@casys/mcp-build123d@0.2.0/server "$@"
    ;;
  calculix)
    exec deno run --config /opt/engineering-toolchain/deno.json --allow-all --cached-only --frozen jsr:@casys/mcp-calculix@0.2.0/server "$@"
    ;;
  *)
    echo "Usage: <syson|build123d|calculix> --port=<port> --hostname=<hostname>" >&2
    exit 64

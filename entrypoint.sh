...
case "$SERVER" in
  syson)
    exec deno run --allow-all --minimum-dependency-age=0 jsr:@casys/mcp-syson@0.3.0/server "$@"
    ;;
  build123d)
    exec deno run --allow-all --minimum-dependency-age=0 jsr:@casys/mcp-build123d@0.1.1/server "$@"
    ;;
  calculix)
